...
class VideoThread(QThread):
    ...
    def __init__(self, video_path):
        ...
        #"/home/ezequiel/Dropbox/Ezequiel/Bayer_solutions/ia/yolov5/eduardo1.mp4"
        self.running = True
    ...
    def postprocess(self, output, img_orig,
                    ...
                    band_px=250):   # <-- NUEVO: ancho de banda en píxeles
        """
        Solo dibuja y devuelve puntos dentro de la banda central:
        |cx - x_center| <= band_px
        Devuelve: (img_orig, n, pts)
        """
        ...
        # filtrar cultivo + confianza
        keep = (conf >= conf_th) & (cls_id == cultivo_id)
        idx = np.where(keep)[0]
        if idx.size == 0:
            return img_orig, 0, []

        # más candidatos porque luego filtramos por banda
        idx = idx[np.argsort(conf[idx])[::-1]][:max_draw * 8]
        ...
        for i in idx:
            ...
            # ✅ FILTRO DE BANDA: si está fuera, NO dibujar, NO agregar
            if abs(cx - x_center) > band_px:
                continue

            pts.append((cx, cy))

            # dibujar SOLO dentro de la banda
    ...
    def run(self):
        ...
        # ---------- parámetros B+ ----------
        deadband_px = 45
        band_px = 250          # achicá esto si toma "más surcos"
        min_pts_total = 10
        min_pts_side = 6       # mínimo por lado (izq/der)
        N_side = 25            # cantidad máxima de puntos por lado
        hold_frames = 5
        ...
        try:
            ...
            while self.running:
                ...
                frame_out, n, pts = self.postprocess(output, frame_raw, band_px=band_px)
                ...
                # línea central violeta
                cv2.line(frame_out, (int(x_center), 0), (int(x_center), H), (255, 0, 255), 2)

                # banda central (visual)
                cv2.line(frame_out, (int(x_center - band_px), 0), (int(x_center - band_px), H), (255, 0, 255), 1)
                cv2.line(frame_out, (int(x_center + band_px), 0), (int(x_center + band_px), H), (255, 0, 255), 1)

                cmd = "NONE"

                if len(pts) >= min_pts_total:
                    # 1) FILTRO POR BANDA (achica el ancho de detección)
                    pts_near = [p for p in pts if abs(p[0] - x_center) <= band_px]

                    # fallback si quedó muy vacío
                    use_pts = pts_near if len(pts_near) >= min_pts_total else pts

                    # 2) separar por lado respecto al centro
                    left_all = [p for p in use_pts if p[0] < x_center]
                    right_all = [p for p in use_pts if p[0] > x_center]

                    # 3) quedarse con la hilera MÁS cercana al centro en cada lado
                    # izquierda: puntos con x más grande (pegados al centro)
                    left_all.sort(key=lambda p: -p[0])
                    # derecha: puntos con x más chico (pegados al centro)
                    right_all.sort(key=lambda p: p[0])

                    left = left_all[:N_side]
                    right = right_all[:N_side]

                    if len(left) >= min_pts_side and len(right) >= min_pts_side:
                        # 4) ajustar líneas con esos puntos
                        ...
                        # dibujo líneas (opcional debug)
                        draw_line(frame_out, mL, bL, y_top, y_bot, (0, 255, 255), 2)
                        draw_line(frame_out, mR, bR, y_top, y_bot, (0, 255, 255), 2)

                        # 5) centro del pasillo en y_ref
                        xL = mL * y_ref + bL
                        xR = mR * y_ref + bR
                        x_mid = 0.5 * (xL + xR)
                        error = x_mid - x_center

                        # punto centro del pasillo (grande y rojo)
                        cv2.circle(frame_out, (int(x_mid), y_ref), 20, (0, 0, 255), -1)
...
class VisionApp(QWidget):
    ...
    def __init__(self, video_path):
        ...
        self.boton1 = QPushButton("www.terod.ai")
        self.boton1.clicked.connect(self.abrir_terod)
