...
class VideoThread(QThread):
    frame_signal = pyqtSignal(np.ndarray)  # Señal que emite el frame
    ...
    def __init__(self, video_path):
        ...
        self.running = True
        self.model = None  # Inicializamos el modelo en None

        # Cargar el modelo SOLO una vez en la inicialización
        self.load_model()
         # Iniciar la cámara en el hilo antes de la interfaz
    ...
    def load_model(self):
        ...
        try:
            ...
            # Cargar el modelo asegurando que está en CPU o GPU según corresponda
    ...
    @staticmethod
    def is_camera_available(index):  # Ahora con el decorador
    ...
    def run(self):
        if not VideoThread.is_camera_available(self.video_path):
            print(f"Error: No se pudo abrir la cámara en el índice {self.video_path}.")
            return
        ...
        while self.running:
            ...
            if not ret:
                break
            # Invertir de BGR a RGB para la inferencia (opcional en YOLOv5)
            ...
            # Contar detecciones
            num_detecciones = len(results.xyxy[0])    

            
            
            if num_detecciones > 0:
                evento_valvula_izquierda.set()
               # opcional: asegurarse que la otra está apagada
            else:
            ...
            # Agregar texto con cantidad de detecciones y estado internet en la imagen
            texto = f"Detecciones: {num_detecciones}                             Internet: {self.estado_internet}"
            cv2.putText(
                frame_with_boxes,
                texto,
                (10, 20),  # posición (x, y)
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,              # tamaño de fuente (ajustalo a gusto)
                (0, 255, 0),    # color (verde)
                2               # grosor
            )
            ...
            self.frame_signal.emit(frame_with_boxes)  # Enviamos el frame con detecciones
